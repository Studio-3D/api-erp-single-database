...
class CompositionBienController extends Controller
{
    ...
    public function index(Request $request)
    {
        if (RoleHelper::ACSup_RC()||RoleHelper::NotaireRespoL()||RoleHelper::Comptable()) {
            ...
            $query = CompositionBien::on('temp');
            if ($bien_id) {
                $query->where('bien_id', $bien_id);
            }
            ...
            if ($request->filled('nbre_salons')) {
                $query->where('nbre_salons', 'like', '%' . $request->input('nbre_salons') . '%');
            }
            if ($request->filled('nbre_cuisines')) {
                $query->where('nbre_cuisines', 'like', '%' . $request->input('nbre_cuisines') . '%');
            }
