...
class NotificationController extends Controller
{
    ...
    public function index(Request $request,$projet_id)
    {
        if(Auth::guard('api')->check()){
            ...
            foreach($data_get->original as $key => $v){
                ...
                if($key=='notifications_new'){
                    $notifications_new=$v;
                }
            }

            return response()->json(['all_notifications'=>$all_notifications,'notifications_new'=>$notifications_new],200);
        }
    ...
    public function get_relances_visites(Request $request,$projet_id)
    {
        if (Auth::guard('api')->check() && RoleHelper::ACSup()) {
           ...
           return response()->json(['rel_visites' => $rel_visites]);
        }
         else{
            return response()->json(['error' => 'Unauthorized'], 401);
         }
    ...
    public function get_relances_menu(Request $request,$projet_id)
    {
        if (Auth::guard('api')->check() && RoleHelper::ACSup()) {
           ...
           return response()->json(['rel_visites' => $rel_visites,'rdv_visites' => $rdv_visites]);
        }
         else{
            return response()->json(['error' => 'Unauthorized'], 401);
         }
    ...
    public function get_notifications(Request $request,$projet_id){
        if (Auth::guard('api')->check() && RoleHelper::ACSup()) {
            ...
            if(RoleHelper::AdminSup()){
               ...
               $notifications_new =$i;
               $notification_new=Notification::on('temp')->where('projet_id',$projet_id)->get();
            }else{
           ...
           return response()->json(['all_notifications' => $all_notifications,'notifications_new'=>$notifications_new]);
        }
         else{
            return response()->json(['error' => 'Unauthorized'], 401);
         }
